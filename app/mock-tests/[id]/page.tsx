...
import { 
  Clock, Award, FileText, ChevronRight, ChevronLeft, Bookmark,
  BookmarkCheck, CheckCircle2, AlertTriangle, RefreshCw, BarChart2, LayoutDashboard
} from "lucide-react";
...
export default function MockTestDetailPage({ params }: { params: { id: string } }) {
  ...
  // Timer Tick
  useEffect(() => {
    if (loading || isCompleted || !test) return;
  ...
  if (loading || !test) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center text-slate-400">
        Initializing mock test simulator...
      </div>
    );
  }
  ...
  // 1. POST TEST COMPLETION VIEW
  if (isCompleted) {
    const accuracy = Math.round((score / test.totalQuestions) * 100);
      ...
      <div className="max-w-4xl mx-auto px-4 py-10 space-y-8 text-left">
        ...
        <div className="p-8 rounded-3xl bg-slate-900 text-white dark:bg-slate-950/80 border border-slate-800 shadow-xl space-y-6 text-center relative overflow-hidden">
          ...
          <div className="space-y-2">
            <span className="text-xs font-bold text-blue-400 uppercase tracking-widest block">Mock Test Submitted</span>
            <h1 className="text-2xl sm:text-3xl font-extrabold font-heading">{test.title}</h1>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-2xl mx-auto pt-4 border-t border-slate-800">
            <div className="p-4 rounded-2xl bg-slate-850 dark:bg-slate-900/60 border border-slate-800">
              ...
              <span className="text-xl font-bold text-slate-100">{score} / {test.totalQuestions}</span>
            </div>
            <div className="p-4 rounded-2xl bg-slate-850 dark:bg-slate-900/60 border border-slate-800">
              <span className="text-[10px] text-slate-400 block uppercase font-medium">Accuracy</span>
              <span className={`text-xl font-bold ${accuracy >= 75 ? "text-green-400" : "text-amber-400"}`}>{accuracy}%</span>
            </div>
            <div className="p-4 rounded-2xl bg-slate-850 dark:bg-slate-900/60 border border-slate-800">
              <span className="text-[10px] text-slate-400 block uppercase font-medium">XP gained</span>
              <span className="text-xl font-bold text-blue-400">⚡ +{xpEarned}</span>
            </div>
            <div className="p-4 rounded-2xl bg-slate-850 dark:bg-slate-900/60 border border-slate-800">
              <span className="text-[10px] text-slate-400 block uppercase font-medium">Coins Earned</span>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                    {q.options.map((opt, oIdx) => {
                      ...
                      let cardStyle = "border-slate-200 dark:border-slate-700 text-slate-650 dark:text-slate-400";
                      if (isCorrectOpt) {
                  ...
                  <div className="p-4 rounded-xl bg-blue-500/5 dark:bg-blue-950/20 border-l-4 border-blue-500 text-xs text-slate-500 leading-relaxed">
                    <span className="font-bold text-slate-700 dark:text-slate-200 block mb-1">Answer Explanations:</span>
                    {q.explanation}
                  </div>
                </div>
              );
  ...
  // 2. ACTIVE MOCK TEST SESSION VIEW
  return (
    <div className="max-w-7xl mx-auto px-4 py-10 text-left grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div className="lg:col-span-8 space-y-6">
        ...
        <div className="flex justify-between items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-sm">
          <div>
            <span className="px-2 py-0.5 rounded text-[9px] bg-blue-500/10 text-blue-600 dark:text-blue-400 font-bold uppercase tracking-wider">
              {test.examName} Full Mock
            </span>
            <h1 className="font-bold text-sm text-slate-800 dark:text-slate-200 block truncate max-w-sm sm:max-w-md mt-1">
              {test.title}
            </h1>
          </div>
          <button
            ...
            className={`p-2 rounded-xl border transition-all ${
              isBookmarked
                ...
                : "border-slate-200 dark:border-slate-700/80 text-slate-400 hover:text-slate-650"
            }`}
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700/60 p-6 sm:p-8 rounded-3xl shadow-premium space-y-6">
          <div className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Question {currentIdx + 1} of {test.totalQuestions}</div>
      ...
      {/* Right panel: Clock timer & Navigator sheet grid */}
      <div className="lg:col-span-4 space-y-6">
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700/60 p-6 rounded-2xl shadow-premium text-center space-y-3">
          ...
          <span className={`text-2xl font-bold font-mono tracking-wider ${timeLeft <= 300 ? "text-red-500 animate-pulse" : "text-slate-800 dark:text-white"}`}>
            {formatTime(timeLeft)}
          </span>
        </div>

        {/* Bubble Grid Navigation sheet */}
        <div className="bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700/60 p-6 rounded-2xl shadow-premium text-left space-y-4">
          <h3 className="font-heading font-bold text-sm text-slate-850 dark:text-white flex items-center gap-1.5">
            <FileText size={16} /> Question Console Matrix
          </h3>
